...
var tomorrowCmd = &cobra.Command{
	...
	Short: "Get tomorrow's weather forecast for a location",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if hour != 0 {
			fmt.Printf("Getting tomorrow's weather for %s at %d:00...\n", locations[0].DisplayName, hour)
			...
			table.RenderHourlyForecast(forecast, time.Now().AddDate(0, 0, 1), hour, cfg.ShowCloud, cfg.ShowHumidity, cfg.ShowDewpoint, cfg.ShowBeaufort, cfg.ShowUV)
		} else {
			fmt.Printf("Getting tomorrow's weather for %s...\n", locations[0].DisplayName)
...
func init() {
	tomorrowCmd.Flags().Int("hour", 0, "Specify the hour for the forecast")
